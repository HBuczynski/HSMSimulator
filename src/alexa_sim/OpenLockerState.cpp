...
OpenLockerState::OpenLockerState(const std::string &name, shared_ptr<State> parent)
        : State(name, parent)
{}

void OpenLockerState::initializeUserQueue(std::shared_ptr<communication::MessageQueueWrapper> queue)
{
    userQueue_ = queue;
}
